...
bestsolution = load("0+15du_doubleweight_alpha=0_ test1.mat");
bestsolution = struct2array(bestsolution);
ang_left = -24;                   
ang_right = 24;                    
L = 10001;  

st_ang_all = -17.9:0.1:17.9;
PSLL_all = zeros(1,length(st_ang_all));
for i = 1:length(st_ang_all)
    ang = st_ang_all(i);
    [theta, AF] = AF_grid(bestsolution, ang_left, ang_right, L, K, ang,space);
    PSLL_all(i) = PSLL(AF);
end
%%
bestsolution2 = load("bestsolution_640_32.1.csv");
PSLL_all_2 = zeros(1,length(st_ang_all));
for i = 1:length(st_ang_all)
    ang = st_ang_all(i);
    [theta, AF] = AF_grid(bestsolution2, ang_left, ang_right, L, K, ang,space);
...
% 假设你已有 x 和 y
figure;
st_ang_all = round(st_ang_all,1);
PSLL_all = round(PSLL_all,1);
plot(st_ang_all, PSLL_all, 'b-', 'LineWidth', 1.5); % 蓝色实线，线宽1.5

xlabel('Steering angle', 'FontSize', 10, 'Interpreter', 'latex');
ylabel('PSLL', 'FontSize', 10, 'Interpreter', 'latex');
title('PSLL under different steering angle','FontSize',10, 'Interpreter', 'latex');
...
% 设置图框大小适用于IEEE（一般为宽8.8cm即~3.5英寸）
set(gcf, 'Units', 'inches', 'Position', [1, 1, 3.5, 2.5]); % [x y width height]

% 去除图框外边白边
set(gca, 'LooseInset', max(get(gca, 'TightInset'), 0.02));
%% 2D1D第十一行 和 普通 645的结果作对比
% 创建图形
figure;

% 画第一条曲线
plot(st_ang_all, PSLL_all, '-b', 'LineWidth', 1.5); 
hold on;

% 画第二条曲线
plot(st_ang_all, PSLL_all_2, '--r', 'LineWidth', 1.5);

% 设置坐标轴标签，字体大小和字体类型符合 IEEE 格式
xlabel('Steering angles', 'FontSize', 10, 'FontName', 'Times New Roman');
ylabel('PSLL', 'FontSize', 10, 'FontName', 'Times New Roman');
title('PSLL under different steering angles')
% 图例，字体设置
legend({'Considering steering process', 'Without considering steering process'}, 'FontSize', 9, 'FontName', 'Times New Roman', 'Location', 'best');
...
% 设置图形大小符合论文要求（比如 3.5 inch 宽，2.5 inch 高）
